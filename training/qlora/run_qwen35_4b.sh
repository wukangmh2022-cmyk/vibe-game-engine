...
if FOUND_PYTHON="$(qlora_find_python "$ROOT" 1)"; then
  ...
  echo "  bash $SCRIPT_DIR/prepare_qwen35_4b.sh" >&2
  qlora_report_missing "${PYTHON_BIN:-python}" 1
  exit 2
fi
if [[ ! -f "$MODEL_DIR/config.json" ]] || ! compgen -G "$MODEL_DIR/*.safetensors" >/dev/null; then
  echo "Base model incomplete at $MODEL_DIR. Prepare/download the 4B base model in no-GPU mode first, or pass MODEL_DIR=/path/to/model." >&2
  exit 2
fi

cd "$ROOT"
exec "$QLORA_PYTHON" "$SCRIPT_DIR/train_qwen35_4b.py" \
