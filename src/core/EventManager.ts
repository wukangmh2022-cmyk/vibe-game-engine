...
export class EventManager implements IEventManager {
  ...
  async emit(event: string, data?: any): Promise<void> {
    // Debug log for event emissions (gated by localStorage.DEBUG_EMIT === '1')
    try {
      const g: any = (typeof window !== 'undefined' ? (window as any) : (globalThis as any));
      const dbg = g?.localStorage?.getItem?.('DEBUG_EMIT') === '1';
      if (dbg && console && console.info) console.info('[emit]', event, data);
    } catch {}
    // 记录事件历史
