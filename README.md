...
要让模型稳定产出能直接试玩的复杂真实关卡，我会先从已玩通的关卡中采集“长需求 -> 多事件关卡补丁 -> 浏览器回放”的完整轨迹，用 QLoRA-SFT 预热它对资源、事件和指令依赖的基本掌握；随后按单事件、交互、跨事件、完整关卡逐级放长任务。强化阶段不会把“写得像 JSON”当奖励，而是以 JSON/资源/依赖契约、真实 runtime 执行、浏览器操作 oracle、目标状态完成和无死事件为硬奖励，语义 judge 只做辅助门槛。若这些自动奖励足够稳定，会对同一需求采样多份候选并用 GRPO 优化相对完成率；若后续积累的是可靠的人类偏好对，则用 DPO 补充玩法取舍；只有确实需要学习长时价值估计时才考虑 PPO。每一轮升级都以 held-out 运行通过率为准，出现回归就回退到上一个 adapter 和数据版本。
