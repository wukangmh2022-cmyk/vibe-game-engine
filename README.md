...
Vibe Game Engine 是一套从运行时到编辑器都自主实现的 2D 游戏创作引擎。它把场景、资源、事件和指令组织为可运行的 JSON：开发者可以继续用代码扩展，创作者也可以在浏览器里完成关卡制作，而不必把每个玩法流程写死在业务代码中。

- **可视化、无代码关卡编辑**：在 Web 编辑器中拖放和管理资源，用指令树编排画面、动画、音频、交互、条件与事件，并立即在 Pixi 预览画布中试玩。常见的答题、选择、拖拽、区域命中和过关流程无需手写脚本。
- **同一份数据驱动运行时**：编辑器保存的 JSON 可以直接交给游戏运行时执行。资源引用、状态、事件和指令处理器共享一套契约，编辑、预览、导出与实际运行不会各自维护一份玩法逻辑。
- **面向关卡创作的 AI**：AI 面板读取当前画布、资源和已有指令，生成可写入关卡的 `commands + extra_events`。配套的微调模型以审计过的高质量中文“需求 -> 指令 JSON”数据训练，并经过资源、依赖关系和真实运行时验证，重点降低资源引用、嵌套事件和多步骤流程的错误率。
