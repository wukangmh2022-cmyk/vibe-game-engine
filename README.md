...
Vibe Game Engine 是一个从游戏运行时到 Web 编辑器都自主实现的 2D 游戏创作项目。该项目将场景、资源、事件和指令组织为可运行的 JSON：既可由开发者继续用代码扩展，也支持在浏览器中直接完成关卡制作。

该项目的编辑器用于无代码关卡搭建。图片、音频和动画可以直接放入画面，再通过指令树编排出现、移动、播放、点击、拖拽、条件和事件，并在 Pixi 预览画布中立即试玩。答题、选项、拖放到目标区域和过关等常见流程无需先写脚本。

该项目也在编辑器中集成了 AI 关卡生成：模型读取当前画布、已有指令和可用资源，生成可直接写回关卡的 `commands + extra_events`。配套模型使用经过资源、依赖关系和真实运行时检查的中文“需求 -> 指令 JSON”数据训练，重点保证生成结果可以实际执行，而不只是形式上像 JSON。
